...
class Like {
  ...
  events() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(".like-box").on("click", this.ourClickDispatcher.bind(this)); // easy way of pointing to our class/object
  }
...
class MyNotes {
  ...
  //Methods will go here
  deleteNote(e) {
    var thisNote = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents("li"); //to select li element - the parent of the delete button we clicked on
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      beforeSend: xhr => {
        xhr.setRequestHeader('X-WP-Nonce', universityData.nonce); // to pass our 'nonce' (from functions.php) to be authorized to do the delete 
      },
...
class Search {
  ...
  typingLogic() {
    if (this.searchField.val() != this.previousValue) {
      // to reset timeout on every input
  ...
  getResults() {
    // Get JSON from the search URL created in /inc/search-route.php and pass data to frontend
                ...
                ${results.professors.map(item => `
                  <li class="professor-card__list-item">
                    <a class="professor-card" href='${item.permalink}'>
                      <img class="professor-card__image" src="${item.image}" alt="img">
                      <span class="professor-car__name">${item.title}</span>
                    </a>
                  </li>
                `).join('')}
    ...
    //when() -> include json request that will run Asynchronously - on completion call then()
    // $.when(
  ...
  openOverlay() {
    ...
    //to open the overlay if the JS is functioning; if not, it will go to -> page-search.php
    return false;
  }
  closeOverlay() {
